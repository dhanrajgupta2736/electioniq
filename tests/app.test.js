...
describe('Timeline Rendering', () => {
  ...
  test('should render all 5 timeline steps', () => {
    ...
    expect(rendered[0].querySelector('.step-title').textContent).toContain('Voter Registration');
    expect(rendered[4].querySelector('.step-title').textContent).toContain('Results');
  });
...
describe('Chat API Response Handling', () => {
  ...
  test('should validate message input', () => {
    const validateMessage = (msg) => {
      if (!msg || typeof msg !== 'string') return { valid: false, error: 'Invalid message' };
      const trimmed = msg.trim();
      if (trimmed.length === 0 || trimmed.length > 1000) return { valid: false, error: 'Length error' };
      return { valid: true };
    };
...
describe('Language Switching', () => {
  const translations = {
    en: { title: 'ElectionIQ', subtitle: 'Your Smart Election Guide', send_btn: 'Send' },
    hi: { title: 'ElectionIQ', subtitle: 'आपका स्मार्ट चुनाव गाइड', send_btn: 'भेजें' },
    mr: { title: 'ElectionIQ', subtitle: 'तुमचा स्मार्ट निवडणूक मार्गदर्शक', send_btn: 'पाठवा' }
  };
...
describe('Error States and Fallback', () => {
  test('should provide fallback news on API failure', () => {
    const fallbackNews = [
      { title: 'Election Commission of India', link: 'https://eci.gov.in', snippet: 'Official portal', source: 'eci.gov.in' }
    ];
  ...
  test('should validate calendar date creation', () => {
    const validateCalendarInput = (title, date) => {
      if (!title || !date) return { valid: false, error: 'Title and date required' };
      if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) return { valid: false, error: 'Invalid date format' };
      return { valid: true };
    };
