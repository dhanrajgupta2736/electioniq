...
| 🗺️ **Polling Station Finder**  | 15+ major Indian cities (incl. expanded Nashik data) with map zoom, Transit Layer & live default view |
...
| 📰 **Election News/Resources** | Real-time election resources and a 24/7 Live YouTube News embed                                       |
| 🌗 **Light/Dark Theme**        | Toggle with animated switch, localStorage persistence & OS preference detection                       |
| 🔐 **Firebase Auth**           | Secure "Sign in with Google" OAuth popup for saving quiz scores to a leaderboard                      |
...
- **Backend:** Node.js with Express.js (Optimized with ETag and 1-day max-age static caching)
- **AI:** Google Gemini API (gemini-2.0-flash)
- **Security:** Helmet (with strict-origin-when-cross-origin), CORS, XSS sanitization, rate limiting
...
| **YouTube Embeds**               | Provides a 24/7 continuous Live Election News broadcast                  |
| **Google Analytics 4 (gtag.js)** | Tracks page views, quiz completions, chat messages, language switches    |
| **Google Fonts**                 | Poppins (headings) + Noto Sans (multilingual body text)                  |
| **Firebase Auth**                | Manages official Google OAuth popup and secure user sessions             |
| **Firebase Firestore**           | Persists chat history and quiz leaderboard scores securely               |
| **Google Maps JavaScript API**   | Live interactive map with Transit Layer toggles, markers, and directions |
...
| ----------------------- | ---------- | -------------------------------------------------- |
| `FIREBASE_API_KEY`      | Yes        | Firebase config API Key for Google Auth            |
| `FIREBASE_PROJECT_ID`   | Yes        | Firebase project ID                                |
| `GOOGLE_MAPS_API_KEY`   | Optional   | Required for Transit Layer and precise map zooming |
...
_\*The app works without API keys using built-in fallback responses. Add keys for full AI-powered functionality._
...
- **Graceful degradation:** Every feature has a fallback. No API key? The app still works with pre-built responses and iframe embeds.
- **Rate limiting:** Chat endpoint is limited to 20 requests/minute to prevent abuse.
- **Optimized Delivery:** Express serves static files with `Cache-Control: max-age=1d` and `ETag` enabled for performance.

### Design Decisions

- **Dark glassmorphism theme** with a civic color palette (saffron, blue, white, green) for a premium, modern look
- **Official Google OAuth Popup** deeply integrated using Firebase Auth rather than the legacy GIS script
...
3. **Polling Finder:** Uses mock data for station results + Google Maps API with a toggleable public Transit Layer overlay.
4. **Calendar:** Generates Google Calendar URLs with pre-filled event details.
5. **Quiz:** Client-side scoring with immediate feedback, pushing successful scores to the Firestore Leaderboard.
6. **Live Broadcast:** YouTube iframe explicitly configured with `referrerpolicy="strict-origin-when-cross-origin"` for seamless live streaming.
