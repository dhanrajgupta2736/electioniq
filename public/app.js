...
import { getFirestore, collection, addDoc, serverTimestamp, query, orderBy, limit, getDocs } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js";
...
(function () {
  ...
  const mockStations = {
    ...
    'nashik': [
      { name: 'Municipal School, Panchavati', address: 'Panchavati, Nashik 422003', type: 'Municipal School', timing: '7 AM – 6 PM', booth: 'Booth 5-10' },
      { name: 'College Road Community Hall', address: 'College Road, Nashik 422005', type: 'Community Hall', timing: '7 AM – 6 PM', booth: 'Booth 15-20' },
      { name: 'Bonsala Military School', address: 'Rambhoomi, Nashik 422005', type: 'School', timing: '7 AM – 6 PM', booth: 'Booth 22-25' },
      { name: 'Nashik Road Govt School', address: 'Nashik Road, Nashik 422101', type: 'Govt. School', timing: '7 AM – 6 PM', booth: 'Booth 1-8' },
      { name: 'Satpur Industrial Hall', address: 'Satpur MIDC, Nashik 422007', type: 'Community Hall', timing: '7 AM – 6 PM', booth: 'Booth 10-18' }
    ],
    '422005': [{ name: 'College Road Community Hall', address: 'College Road, Nashik 422005', type: 'Community Hall', timing: '7 AM – 6 PM', booth: 'Booth 15-20' }, { name: 'Bonsala Military School', address: 'Rambhoomi, Nashik 422005', type: 'School', timing: '7 AM – 6 PM', booth: 'Booth 22-25' }],
    '422003': [{ name: 'Municipal School, Panchavati', address: 'Panchavati, Nashik 422003', type: 'Municipal School', timing: '7 AM – 6 PM', booth: 'Booth 5-10' }],
    '422101': [{ name: 'Nashik Road Govt School', address: 'Nashik Road, Nashik 422101', type: 'Govt. School', timing: '7 AM – 6 PM', booth: 'Booth 1-8' }],
    '422007': [{ name: 'Satpur Industrial Hall', address: 'Satpur MIDC, Nashik 422007', type: 'Community Hall', timing: '7 AM – 6 PM', booth: 'Booth 10-18' }],
  ...
  async function loadConfig() {
    try { const res = await fetch('/api/config'); return await res.json(); } catch { return {}; }
  }
  ...
  function renderTimeline() {
    ...
    timelineSteps.forEach((step, i) => {
      ...
      el.addEventListener('click', () => { toggleStep(step.num); trackEvent('timeline_click', { step: step.num }); });
  ...
  function setupPolling() {
    ...
    // GA tracking
    btn.addEventListener('click', () => trackEvent('find_station', { location: input.value }));
  }
  ...
  function findStation() {
    ...
    if (window.google && window.google.maps && document.getElementById('google-map')) {
      ...
      if (el) el.innerHTML = '<iframe width="100%" height="350" style="border:0;border-radius:12px" allowfullscreen loading="lazy" src="https://www.google.com/maps?q=' + q + '&z=15&output=embed"></iframe>';
    }
  }
  ...
  function showQuizResults() {
    ...
    const userName = window._electionIQUser ? window._electionIQUser.name : 'Anonymous';
    saveScoreToFirestore(userName, score, total);
    fetch('/api/log-score', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: userName, score, total, percentage: pct }) }).catch(() => {});
  ...
  function reportWebVitals() {
    ...
    script.onload = () => {
      webVitals.onCLS(v => trackEvent('web_vitals', { metric: 'CLS', value: v.value }));
      webVitals.onFID(v => trackEvent('web_vitals', { metric: 'FID', value: v.value }));
      webVitals.onLCP(v => trackEvent('web_vitals', { metric: 'LCP', value: v.value }));
    };
  ...
  function trackEvent(name, params) { if (window.gtag) gtag('event', name, params || {}); }
  ...
  async function loadLeaderboard() {
    ...
    if (!list) return;
    if (!db) { list.innerHTML = '<p style="color:var(--text-secondary);font-size:0.85rem;">Connect Firebase to see leaderboard.</p>'; return; }
    try {
      const q = query(collection(db, 'leaderboard'), orderBy('percentage', 'desc'), limit(5));
      const snap = await getDocs(q);
      if (snap.empty) { list.innerHTML = '<p style="color:var(--text-secondary);font-size:0.85rem;">No scores yet!</p>'; return; }
      const medals = ['\u{1F947}','\u{1F948}','\u{1F949}','4\uFE0F\u20E3','5\uFE0F\u20E3'];
      let html = ''; let r = 0;
      snap.forEach(d => { const v = d.data(); html += '<div class="lb-entry"><span class="lb-rank">' + medals[r++] + '</span><span class="lb-name">' + v.name + '</span><span class="lb-score">' + v.score + '/' + v.total + '</span></div>'; });
      list.innerHTML = html;
    } catch(e) { list.innerHTML = '<p style="color:var(--text-secondary);font-size:0.85rem;">Could not load leaderboard.</p>'; }
  }

  // ===== GOOGLE IDENTITY =====
  function setupGoogleIdentity(clientId) {
    const customBtn = document.getElementById('google-signin-btn');
    const container = document.getElementById('google-btn-wrapper') || createGoogleWrapper();
    
    // Real Google Sign-In Flow via Firebase Auth
    if (customBtn) {
      customBtn.addEventListener('click', async () => {
        if (!auth) {
          alert('Firebase Auth is not initialized. Please ensure FIREBASE_CONFIG is set in your .env file.');
          return;
        }
        try {
          ...
          if (window.gtag) gtag('event', 'user_signin', { method: 'Firebase_Google' });
        } catch (error) {
    ...
    function createGoogleWrapper() {
      ...
      document.getElementById('google-signin-container').insertBefore(w, document.getElementById('user-profile'));
      return w;
    }
  ...
  window.initDefaultMap = function() {
    ...
    currentMap = new google.maps.Map(el, { zoom: 4, center: defaultCenter, mapTypeControl: false });
  ...
  function setupTransitLayerToggle() {
    ...
    if (btn) {
      ...
      newBtn.addEventListener('click', () => {
        ...
        if (typeof trackEvent === 'function') trackEvent('toggle_transit_layer');
      });
    }
  }
  ...
  function loadMapsAPI(key) {
    const s = document.createElement('script');
    s.src = 'https://maps.googleapis.com/maps/api/js?key=' + key + '&libraries=places&callback=initDefaultMap';
  ...
  function initPollingMap(loc, stations) {
    const el = document.getElementById('google-map');
    if (!el || !window.google || !window.google.maps) return;
    new google.maps.Geocoder().geocode({ address: loc + ', India' }, (res, st) => {
      if (st !== 'OK' || !res[0]) return;
      const ctr = res[0].geometry.location;
      
      // Update global map reference
      currentMap = new google.maps.Map(el, { zoom: 15, center: ctr, mapTypeControl: false });
      ...
      stations.forEach((s, i) => {
        const m = new google.maps.Marker({ position: { lat: ctr.lat() + i*0.003, lng: ctr.lng() + i*0.004 }, map: currentMap, title: s.name });
        const info = new google.maps.InfoWindow({ content: '<strong>' + s.name + '</strong><br>' + s.address + '<br><a href="https://www.google.com/maps/dir/?api=1&destination=' + encodeURIComponent(s.address) + '" target="_blank">Get Directions</a>' });
